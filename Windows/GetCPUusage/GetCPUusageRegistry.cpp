...
#define UNICODE
#include <stdio.h>
#include <global.h> //DEBUG_COUT()
//#include <strsafe.h> //StringCchCopyN(...) , StringCchPrintf(...) : MSVC-specific
//#include <tchar.h>

#include <windows.h>
//#include <windef.h> //DWORD etc. in winperf.h
//#include <winperf.h> //PERF_COUNTER_BLOCK etc.
//#include <winreg.h> //HKEY_PERFORMANCE_DATA, RegCloseKey(...), RegQueryValueEx(...)
  ...
  //if( byCoreID == 0 )
  {
    ...
    if( ( m_dwAtMask2ndTimeCPUcoreMask >> byCoreID ) & 1 )
    {
      ...
      //for (DWORD i = 0; i < 5; i++)
      {
        ...
        g_pPerfDataHead = (LPBYTE) GetPerformanceData(
          L"238",
          INIT_OBJECT_BUFFER_SIZE
          );
        if (NULL == g_pPerfDataHead)
        {
          DEBUG_WPRINTFN(L"GetPerformanceData in loop failed.\n");
          goto cleanup;
        }
//        m_stdowstringstream << (WORD) byCoreID ;
        DEBUG_WPRINTFN(L"core ID: %u", (WORD) byCoreID )
        //m_stdowstringstream.flush() ;

        //The wstringstream does not work with MinGW.
//        std::wstring stdwstr = m_stdowstringstream.str() ;
//        LPWSTR lpwstrCPUcoreNumber //= (LPWSTR) stdwstr.c_str() ;
//          ;
        //http://www.velocityreviews.com/forums/t281293-integer-to-string.html:
//        wchar_t ar_wch[20] ;
        ...
        fSuccess = GetCounterValues(
          ...
          6,
          //L"0"
          //the core ID/ number as string.
          m_lpwstrCPUcoreNumber , //&Sample2
...
//        m_stdowstringstream.seekp(0) ;
        if ( FALSE == fSuccess )
        {
          DEBUG_WPRINTFN(L"GetCounterValues failed.\n");
          goto cleanup;
        }
        ...
        if (FALSE == fSuccess)
        {
          DEBUG_WPRINTFN(L"DisplayCalculatedValue failed.\n");
          goto cleanup;
        }
      }
//  cleanup:
...
//          free(g_pPerfDataHead);
    ...
    { //Fetch the 1st sample.
      BOOL fSuccess = FALSE;
      m_dwAtMask2ndTimeCPUcoreMask |= ( 1 << byCoreID ) ;
      // Retrieve counter data for the Processor object.
      g_pPerfDataHead = (LPBYTE)GetPerformanceData( L"238",
        INIT_OBJECT_BUFFER_SIZE);
      if (NULL == g_pPerfDataHead)
      {
        //DEBUG_WPRINTFN(L"GetPerformanceData failed.\n");
        goto cleanup;
      }
      m_stdowstringstream << (WORD) byCoreID ;
      //m_stdowstringstream.flush() ;
...
inline LPBYTE GetPerformanceData(LPWSTR pwszSource, DWORD dwInitialBufferSize)
{
  ...
  if (pBuffer)
  {
      ...
      )
    {
      ...
      {
        //DEBUG_WPRINTFN(L"Reallocation error.\n");
        free(pBuffer);
        pBuffer = NULL;
        goto cleanup;
      }
    ...
    {
      //DEBUG_WPRINTFN(L"RegQueryValueEx failed with 0x%x.\n", status);
      free(pBuffer);
      pBuffer = NULL;
    }
  }
  ...
  {
    //DEBUG_WPRINTFN(L"malloc failed to allocate initial memory request.\n");
  }
cleanup:
  ...
  )
{
  ...
  if (NULL == pObject)
  {
      DEBUG_WPRINTFN(L"Object  %d not found.\n", dwObjectIndex);
      goto cleanup;
  }
  ...
  if (NULL == pCounter)
  {
      DEBUG_WPRINTFN(L"Counter %d not found.\n", dwCounterIndex);
      goto cleanup;
  }
  ...
  {
      DEBUG_WPRINTFN(L"GetCounterValues()--Instance \"%s\" not found.\n", pInstanceName);
      goto cleanup;
  }
...
inline PERF_OBJECT_TYPE* GetObject(DWORD dwObjectToFind)
{
  LPBYTE pObject = g_pPerfDataHead +
    ((PERF_DATA_BLOCK*)g_pPerfDataHead)->HeaderLength ;
...
inline PERF_INSTANCE_DEFINITION * GetObjectInstance(
  PERF_OBJECT_TYPE * pObject,
  LPWSTR pInstanceName
  )
{
  PERF_INSTANCE_DEFINITION* pInstance = (PERF_INSTANCE_DEFINITION*)
  ...
  )
{
  ...
  if (CodePage == 0)  // Instance name is a Unicode string
  {
    ...
    //http://msdn.microsoft.com/en-us/library/ms647530%28VS.85%29.aspx:
    //"StringCchCopyN is a replacement for strncpy. StringCchCopyN copies a
    //given number of characters from a source string. The size, in characters,
    //of the destination buffer is provided to the function to ensure that
    //StringCchCopyN  does not write past the end of this buffer."
//    StringCchCopyN(
//      //"Pointer to a buffer which receives the copied characters."
//      wszInstanceName,
//      MAX_INSTANCE_NAME_LEN+1, //"Size of pszDest, in characters. "
//      (LPWSTR)(((LPBYTE)pInstance)+pInstance->NameOffset),
//      //"The maximum number of characters to copy from pszSrc to pszDest."
//      dwLength );
//    tcsncpy(
    wcsncpy(
      wszInstanceName,
  ...
  else  // Convert the multi-byte instance name to Unicode
  {
      ...
      if (FALSE == fSuccess)
      {
          DEBUG_WPRINTFN(L"ConvertNameToUnicode for instance failed.\n");
          goto cleanup;
      }
  }
...
//        , dwLength);
    ...
    else  // Convert the multi-byte instance name to Unicode
    {
        ...
        if (FALSE == fSuccess)
        {
            DEBUG_WPRINTFN(L"ConvertNameToUnicode for parent instance failed.\n");
            goto cleanup;
        }
    }
//    StringCchPrintf(
...
//      wszInstanceName);
    swprintf(
      p_wchName,  //wchar_t *buffer,
      L"%s/%s", //const wchar_t *format [,
      //argument] ...
      wszParentInstanceName,
      wszInstanceName
    );
  ...
  {
//    //http://msdn.microsoft.com/en-us/library/ms647541%28VS.85%29.aspx:
//    //"StringCchPrintf is a replacement for sprintf."
//    StringCchPrintf(
//      p_wchName,
//      MAX_INSTANCE_NAME_LEN+1,
//      L"%s", wszInstanceName
//      );
    swprintf(
      p_wchName,  //wchar_t *buffer,
      L"%s", //const wchar_t *format [,
      wszInstanceName //argument] ...
    );
  }
cleanup:
  return fSuccess;
...
// the value.
inline BOOL DisplayCalculatedValue(
  RAW_DATA * pSample1,
  RAW_DATA * pSample2,
  double & r_dValue
  )
{
  ...
  if (PERF_DELTA_COUNTER == (pSample1->CounterType & PERF_DELTA_COUNTER) &&
      NULL == pSample2)
  {
    //DEBUG_WPRINTFN(L"The counter type requires two samples but only one sample was passed.\n");
    fSuccess = FALSE;
    goto cleanup;
  }
  ...
  if (pSample2 != NULL && pSample1->Data > pSample2->Data)
  {
    ...
    //DEBUG_WPRINTFN(L"Sample1 (%I64u) is larger than sample2 (%I64u).\n", pSample1->Data, pSample2->Data);
    fSuccess = FALSE;
    goto cleanup;
  }
  ...
  switch (pSample1->CounterType)
  {
      ...
      //    //DEBUG_WPRINTFN(L"Display value is %u%s\n", DisplayValue2,
      ...
      //    //    DEBUG_WPRINTFN(L"Display value is %f.\n", DisplayValue);
      ...
      //    //DEBUG_WPRINTFN(L"PERF_SAMPLE_FRACTION--Display value is %f%%.\n", DisplayValue);
      //    r_dValue = DisplayValue ;
      //    break;
      ...
      //    //DEBUG_WPRINTFN(L"PERF_COUNTER_TIMER_INV--Display value is %f%%.\n", DisplayValue);
      //    r_dValue = DisplayValue ;
      //    break;
      ...
      case PERF_100NSEC_TIMER_INV:  // 100*(1- (N1 - N0)/(D1 - D0))
          ...
          //DisplayValue = 100*(1 - (double)numerator/denominator);
          //r_dValue = DisplayValue ;
          //DEBUG_WPRINTFN(L"PERF_100NSEC_TIMER_INV--Display value is %f%%.\n", DisplayValue);
      ...
      //    //DEBUG_WPRINTFN(L"PERF_COUNTER_MULTI_TIMER--Display value is %f%%.\n", DisplayValue);
      //    r_dValue = DisplayValue ;
      //    break;
      ...
      //    denominator = pSample2->Time - pSample1->Time;
      //    DisplayValue = 100*((double)numerator/denominator)/pSample2->MultiCounterData;
      //    //DEBUG_WPRINTFN(L"PERF_100NSEC_MULTI_TIMER--Display value is %f%%.\n", DisplayValue);
      //    r_dValue = DisplayValue ;
      //    break;
      ...
      //    //DEBUG_WPRINTFN(L"PERF_100NSEC_MULTI_TIMER_INV--Display value is %f%%.\n", DisplayValue);
      //    r_dValue = DisplayValue ;
      //    break;

      //case PERF_COUNTER_RAWCOUNT:  // N as decimal
      //case PERF_COUNTER_LARGE_RAWCOUNT:
      //    //DEBUG_WPRINTFN(L"Display value is %I64u.\n", pSample1->Data);
      //    r_dValue = (double) pSample1->Data ;
      //    break;

      //case PERF_COUNTER_RAWCOUNT_HEX:  // N as hexadecimal
      //case PERF_COUNTER_LARGE_RAWCOUNT_HEX:
      //    //DEBUG_WPRINTFN(L"Display value is %I64x.\n", pSample1->Data);
      //    r_dValue = (double) pSample1->Data ;
      //    break;
      ...
      //    //DEBUG_WPRINTFN(L"PERF_LARGE_RAW_FRACTION--Display value is %f%%.\n", DisplayValue);
      //    r_dValue = DisplayValue ;
      //    break;
      ...
      //    //DEBUG_WPRINTFN(L"Display value is %f in seconds.\n", DisplayValue);
      //    r_dValue = DisplayValue ;
      //    break;
      ...
      //default:
      //    //DEBUG_WPRINTFN(L"Counter type not found.\n");
      //    fSuccess = FALSE;
      //    break;
    }
cleanup:
  ...
  )
{
  ...
  switch (pCounter->CounterType)
  {
    ...
    case PERF_SAMPLE_COUNTER:
      ...
      //The following uncommented defines are Not supported in MinGW' winperf.h.
...
//    case PERF_COUNTER_LARGE_QUEUELEN_TYPE:
        ...
        {
          pRawData->Frequency = ((PERF_DATA_BLOCK*)g_pPerfDataHead)->
              PerfFreq.QuadPart;
        }
        break;
...
//    case PERF_COUNTER_LARGE_DELTA:
    ...
    case PERF_100NSEC_TIMER:
      //PERF_100NSEC_TIMER_INV is used for CPU core load calculating:
