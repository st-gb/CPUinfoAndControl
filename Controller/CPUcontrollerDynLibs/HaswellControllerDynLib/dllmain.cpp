...
//#ifdef WESTMERE
  const float lowestMultiplier = 12.0; //The minimum multiplier for Westmere is 9.
//#else
...
/** Calling convention--must be the same as in the DLL function signature that
 *  calls this function?! */
//WINAPI
  ...
  )
{
  ...
  static fastestUnsignedDataType maximumMulti = 0;
...
//  g_byValue1 = Intel::Nehalem::GetMaximumMultiplier(wCoreID, maximumMulti);
  const fastestUnsignedDataType baseMultiplier = (fastestUnsignedDataType)
    Intel::Nehalem::GetTimeStampCounterMultiplier();
  DEBUGN("Dyn Lib--return value of Exe's ReadMSR:" << (WORD) g_byValue1)
  if( g_byValue1 ) //successfully read from MSR
  {
    DEBUGN("Dyn Lib--max multi:" << (WORD) maximumMulti)
#ifdef JUST1MULTIPLIER
    BYTE numMultis = 1;
#else
    const fastestUnsignedDataType lowestMultiplier = Intel::Nehalem::GetLowestMultiplier();
    const fastestUnsignedDataType numMultis = /*maximumMulti*/ baseMultiplier -
      //min. multi - 1
      lowestMultiplier + 1 ;
#ifdef USE_ODCM
    numMultis += NUMBER_OF_ODCM_VALUES; //for OCDM cycles
#endif
#endif
    float * ar_f = new float[numMultis] ;
...
#endif
      DEBUGN("lowest multi: " << lowestMultiplier)
      float fMulti = lowestMultiplier;
//     stdstrstream << "float array addr.:" << ar_f << " avail. multis:" ;
      for( BYTE by = ARRAY_INDEX_FOR_FIRST_MULTIPLIER; by < numMultis ; ++ by )
      {
...
/** @ return the _adjustable_ voltages.
...
EXPORT float * DYN_LIB_CALLING_CONVENTION GetAvailableVoltagesInVolt(
  WORD wCoreID
  , WORD * p_wNum )
{
  DEBUGN( FULL_FUNC_NAME << "--begin")
...
#else //#ifdef USE_ODCM
    * p_fMultiplier = (float) (g_ui32Value1 >> 8);
    * p_fVoltageInVolt = 0.49f + (float) (ui32Value & BITMASK_FOR_LOWMOST_8BIT) * 0.005f;
...
EXPORT void DYN_LIB_CALLING_CONVENTION Init( //I_CPUcontroller * pi_cpu
...
EXPORT WORD DYN_LIB_CALLING_CONVENTION PrepareForNextPerformanceCounting(
 DWORD dwAffMask
...
//    MessageBox(NULL,ss.str().c_str(), "info" , MB_OK ) ;
  ...
  else
  {
    ...
    g_byValue1 =
      ...
      //  "63:33 Reserved"
      IA32_PERF_CTL,
      ((BYTE) fMultiplier) >> 8, // bits 0-31 (register "EAX")
