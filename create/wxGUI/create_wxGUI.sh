...
numCmdLineArgsInclBuildSysgen=`expr $NUM_ARGS_NEEDED_FOR_INCLUDING_COMPILERS + 2`

#IDEA: pass as args: <NAME=VALUE>, e.g. COMMON_SOURCECODE_ROOT_PATH=../common_sourcecode
if [ $NUMBER_OF_SHELL_ARGS -ge $MINIMAL_NUM_ARGS_NEEDED ]; then
...
if [ $NUMBER_OF_SHELL_ARGS -ge $NUM_ARGS_NEEDED ]; then
...
if [ $NUMBER_OF_SHELL_ARGS -ge $NUM_ARGS_NEEDED_FOR_INCLUDING_COMPILERS ]; then
  C_COMPILER=$5
  CPP_COMPILER=$6
fi
if [ $NUMBER_OF_SHELL_ARGS -ge $numCmdLineArgsInclBuildSysgen ]; then
  CMakeBuildSysGenerator=$7
else
  CMakeBuildSysGenerator="Unix Makefiles"
fi
