...
if( type == "GUI")
	set windres_params=--define __WXMSW__ %include_wx_setup_h_dir% %include_wx_include_dir% all_ressources_do_not_requ_admin_GUI.rc 
else if( type=="service")
	set windres_params=all_ressources_requ_admin_service.rc 
...
gcc -E -xc -D__WXMSW__ -DRC_INVOKED -DCOPYRIGHT_STRING=%copyright_string% -DBUILD_WITH_STRING=%built_with_string% -DFILE_VERSION_NUMBER=%file_version_numeric% -DFILE_VERSION_STRING=%file_version_string% %include_wx_setup_h_dir% %include_wx_include_dir% x86IandC_version_info.rc -o %out_file_name_prefix%_preprocessed.rc

windres --use-temp-file %windres_params% all_ressources_do_not_requ_admin_GUI.opc
